...
class DataModule(pl.LightningDataModule):
    ...
    def _filter(
        ...
        dfX = df.filter(regex="^feature_", axis=1)
        dfy = df.filter(regex="^target_", axis=1)
        return dfX, dfy

    def get_xy(self, split: str, dask: bool = False) -> Tuple[np.ndarray]:
        ...
        if split == "train":
            ...
            raise ValueError(
                f"'{split}' is not a supported split. Try 'train' or 'val'"
            )

        if dask:
            return self._filter(dd.read_parquet(fp))
        else:
            return self._filter(pd.read_parquet(fp))
